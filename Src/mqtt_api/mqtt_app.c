...
TimerHandle_t UPGRADETIMEOUTTimer_Handle = NULL;
...
void MqttThread( void ){
    ...
    #define CLAIMUPGRADEINTERVAL                5
    void RequestUpgradePacketTimeout( TimerHandle_t xTimer );
    UPGRADETIMEOUTTimer_Handle   = xTimerCreate((const char* )"RequestCurrentUpgradePacketIndex", (TickType_t )CLAIMUPGRADEINTERVAL*1000,(UBaseType_t )pdFALSE,(void* )2,(TimerCallbackFunction_t)RequestUpgradePacketTimeout);
    if( UPGRADETIMEOUTTimer_Handle == NULL ){
      DEBUG_INFO("UPGRADETIMEOUTTimer_Handle is NULL\r\n");
      while( true );
    }
    xTimerStop(  UPGRADETIMEOUTTimer_Handle, TIMER_WAIT );
...
bool MqttEncryption( MqttSend_t *Ori, SEND_PARA* DestPayload ){
    ...
    if( DestPayload->HANDLE ){
        if( Ori->topic ){
            //encryption payload
            SetPayloadLength( ( uint8_t *)Ori->payload , Ori->payloadlen );
            SetPayloadCRC( (uint8_t *)Ori->payload, Ori->payloadlen );
            if( (uint8_t)Ori->payload[0] == 0x7f && (uint8_t)Ori->payload[1] == 0xef ){
                PayloadEncrypt( ( uint8_t *)Ori->payload + 4, ( Ori->payloadlen - 4 ) , NETKEY, ( uint8_t *)Ori->payload + 4 );
            }else if( (uint8_t)Ori->payload[0] == 0xef && (uint8_t)Ori->payload[1] == 0x7f ){
                PayloadEncrypt( ( uint8_t *)Ori->payload + 4, ( Ori->payloadlen - 4 ) , APPKEY, ( uint8_t *)Ori->payload + 4 );
            }            
					...
					if (rc == 1){
						if ( granted_qos != 0 && granted_qos != 1 && granted_qos != 2 && (submsgid == SUB_INDEXES_TOPIC || submsgid == SUB_MQTT_ID )) {
							...
							if ( submsgid == SUB_MQTT_ID ){
                                extern bool CheckUpgradeState( void );
                                CheckUpgradeState();                                
